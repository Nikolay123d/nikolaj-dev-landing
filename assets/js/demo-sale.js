(function () {
  ...
  const packagePlans = {
    Start: {
      ...
      cta: "Купить Start со скидкой 50%"
    },
    Plus: {
    ...
    Pro: {
      regular: "от 30 000 Kč / от ≈1 200 €",
      discount: "от 15 000 Kč / от ≈600 €",
  ...
  function fillSaleFields(form, sourceCta) {
    ...
    if (message && !message.value.trim()) {
      message.value = active
        ? `Хочу похожий сайт по акции 50%. Demo: ${templateName}.`
        : `Хочу похожий сайт. Demo: ${templateName}.`;
    }
  }
  ...
  function enhanceForms() {
    document.querySelectorAll("form[action*='forminit.com']").forEach((form) => {
      ...
      if (active && !existingNote) {
        ...
        note.innerHTML = `<strong>Заявка по акции 50%</strong><span>До конца предложения: <b data-sale-countdown>03:00:00</b></span><span>Если проект подходит под готовый demo-шаблон, старт от 5 000 Kč / ≈200 €.</span>`;
        form.prepend(note);
      }
      ...
      form.addEventListener("submit", () => {
        ...
        const summaryLines = saleActive
          ? [
            "SALE 50%",
            `Пакет: ${form.querySelector("[name='selected_package']")?.value || "Start"}`,
            `Обычная цена: ${form.querySelector("[name='regular_price']")?.value || "10 000 Kč / ≈400 €"}`,
            `Цена по акции: ${form.querySelector("[name='discount_price']")?.value || "5 000 Kč / ≈200 €"}`,
            `Промокод: ${saleConfig.code}`,
            `Источник кнопки: ${form.querySelector("[name='source_cta']")?.value || "demo_form_submit"}`,
            `Осталось времени по таймеру: ${form.querySelector("[name='sale_remaining']")?.value || "не указано"}`
          ]
          : [
            "Заявка без активной акции",
            `Пакет: ${form.querySelector("[name='selected_package']")?.value || "Start"}`,
            `Цена пакета: ${form.querySelector("[name='package_price']")?.value || "10 000 Kč / ≈400 €"}`,
            `Источник кнопки: ${form.querySelector("[name='source_cta']")?.value || "demo_form_submit"}`
          ];
  ...
  function enhancePackageSelects() {
    ...
    document.querySelectorAll("select[name='selected_package_visible'] option[value]").forEach((option) => {
      ...
      option.textContent = active
        ? `${option.value.toUpperCase()} — акция ${plan.discount} — обычно ${plan.regular}`
  ...
  function enhancePricingCards() {
    ...
    document.querySelectorAll("[data-package]").forEach((button) => {
      ...
      button.textContent = active ? plan.cta : (packageName === "Pro" ? "Обсудить Pro" : `Выбрать ${packageName}`);
      ...
      if (card && active && !card.querySelector(".demo-sale-card-badge")) {
        ...
        badge.textContent = "-50% / 3 часа";
        card.prepend(badge);
      }
      const priceTarget = card?.querySelector("strong, .price, .plan-price");
      if (priceTarget) {
        priceTarget.innerHTML = active
          ? `<span class="demo-sale-new">${plan.discount}</span><span class="demo-sale-old">обычно ${plan.regular}</span>`
  ...
  function addSaleChips() {
    ...
    if (heroContent && !heroContent.querySelector(".demo-sale-chip")) {
      ...
      chip.innerHTML = `<strong>-50%</strong><span>акция на похожий сайт</span><b data-sale-countdown>03:00:00</b>`;
  ...
  function showPopup() {
    ...
    popup.innerHTML = `
      ...
      <div>
        <a class="btn primary" href="#${target.id || "order"}">Заказать похожий по акции</a>
        <button class="btn ghost" type="button">Продолжить смотреть</button>
      </div>
    `;
