(function () {
  ...
  function buildMessage(form, meta) {
    ...
    if (/SALE 50%|Заявка без активной акции|Normal request/i.test(originalMessage)) {
    ...
    if (meta.promoStatus === "ACTIVE") {
      lines.push(
        "",
        "SALE 50%",
        `Promo code: ${meta.promoCode || "SALE50_3H"}`,
  ...
  function buildForminitPayload(form) {
    ...
    const meta = {
      promoCode,
      promoStatus,
      promoLabel: promoStatus === "ACTIVE" ? (value(form, ["promo_label"]) || "Скидка 50% / 3 часа") : "",
      discountPercent,
  ...
  async function submitForminit(form, submitter) {
    ...
    try {
      ...
      showFormError(form, "Forminit отклонил заявку. Проверьте поля и попробуйте ещё раз.");
