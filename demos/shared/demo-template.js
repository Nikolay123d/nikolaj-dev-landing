(function () {
  ...
  const plans = [
    {
      ...
      text: "Готовый demo-шаблон: меняем тексты, фото, контакты, форму заявки и готовим публикацию.",
      extra: "+ домен ~150-300 Kč / год · хостинг 0 Kč для обычного лендинга",
    ...
    },
    {
      ...
      saleEur: "≈400 €",
      badge: "Оптимально",
      text: "Больше блоков, второй язык по задаче, thank-you страница, базовая аналитика и аккуратная доработка.",
      extra: "+ домен ~150-300 Kč / год · хостинг 0 Kč для обычного лендинга",
    ...
    },
    {
      ...
      price: "от 30 000 Kč",
      eur: "от ≈1 200 €",
      salePrice: "от 15 000 Kč",
      saleEur: "от ≈600 €",
      badge: "Под рост",
      text: "Расширенная структура, SEO-основа, аналитика кликов, UTM и подготовка под рекламу или сложнее логику. Объём фиксируется заранее.",
      extra: "+ домен ~150-300 Kč / год · платные сервисы по задаче",
  ...
  const imageStack = (variant = "stack") => `
    <div class="hero-visual ${variant}">
      ...
      <div class="hero-floating-card">
        <strong>${cfg.info[1]?.value || "3-7 дней"}</strong>
  ...
  const heroPanel = () => {
    ...
    const content = `
      ...
      ${saleActive ? `<div class="sale-demo-chip"><strong>-50%</strong><span>акция на похожий сайт</span><b data-sale-countdown>03:00:00</b></div>` : ""}
      <h1>${cfg.title}</h1>
      <p>${cfg.subtitle}</p>
      ${actions}
    ...
    if (["appointment", "search", "booking", "dashboard", "audit", "terminal"].includes(heroMode)) {
      const panelTitle = {
        ...
        search: "Подбор объекта",
        booking: "Проверить даты",
        dashboard: "MVP scope",
        audit: "Audit board",
        terminal: "Bug report"
      }[heroMode];
        ...
        <section class="hero hero-panel">
          ...
          <div class="wrap hero-grid">
            ...
            <aside class="hero-side-panel">
              ...
              <a class="btn primary full" href="#request">Заполнить заявку</a>
            </aside>
          </div>
        </section>
      `;
  ...
  const nav = () => `
    <div class="demo-bar">
      ...
      <a href="../../index.html#catalog">← Назад</a><span>·</span>
      <a class="order" href="#request">Заказать похожий</a>
    </div>
  ...
  const prices = (variant = "") => {
    ...
    return `
      <section id="prices" class="pricing-section ${variant}">
        <div class="wrap">
          <div class="section-head">
            <div><p class="kicker">Стоимость сайта</p><h2>START / PLUS / PRO</h2></div>
            <p>Клиент сразу видит, чем отличается быстрый запуск по demo-шаблону от более глубокой доработки.</p>
          </div>
          <div class="price-grid">
            ${plans.map((plan) => `
              <article class="price-card ${plan.recommended ? "recommended" : ""} ${plan.premium ? "premium" : ""}">
                ${saleActive ? `<span class="sale-mini-badge">-50% сегодня</span>` : ""}
                ...
                <button class="btn ${plan.premium || plan.recommended ? "primary" : "soft"}" type="button" data-package="${plan.key}" data-package-price="${saleActive ? `${plan.salePrice} / ${plan.saleEur}` : `${plan.price} / ${plan.eur}`}" data-regular-price="${saleActive ? `${plan.price} / ${plan.eur}` : ""}" data-source-cta="demo_pricing_${plan.key.toLowerCase()}">${saleActive ? plan.cta : (plan.key === "Pro" ? "Обсудить Pro" : `Выбрать ${plan.title}`)}</button>
              </article>
            `).join("")}
          </div>
          <div class="price-note"><strong>${saleActive ? "Минимальный запуск по акции:" : "Минимальный запуск:"}</strong> ${saleActive ? "START 5 000 Kč / ≈200 €" : "START 10 000 Kč / ≈400 €"} + домен примерно 150-300 Kč / год + бесплатный хостинг.${saleActive ? ` До конца предложения: <b data-sale-countdown>03:00:00</b>.` : ""} Домен, платные формы, платный хостинг, рекламный бюджет, логотип, профессиональные фото, переводы и сложные функции оплачиваются отдельно.</div>
        </div>
      </section>
    `;
  };
  ...
  const gallery = (variant = "photo-gallery") => `
    <section class="gallery-section ${variant}">
      <div class="wrap">
        <div class="section-head">
          <div><p class="kicker">Визуальная подача</p><h2>${galleryTitle()}</h2></div>
          <p>В реальном проекте сюда ставятся настоящие фото бизнеса, команды, работ, пространства или продукта.</p>
        </div>
  ...
  function galleryTitle() {
    if (layout === "gallery") return "Портфолио и визуальная история";
    if (layout === "estate") return "Объекты, детали и доверие";
    if (layout === "catalog") return "Продукты и витрина";
    if (layout === "garage") return "Работы, детали и сервис";
    return "Фото, оффер и CTA под нишу";
  }
        ...
        <div class="panel">
          <form action="https://forminit.com/f/kshbr37bfe4" method="POST">
            ...
            ${saleActive ? `<div class="sale-form-note">
              ...
              <span>Если проект подходит под готовый demo-шаблон, старт от 5 000 Kč / ≈200 €.</span>
            </div>` : ""}
            <select name="selected_package_visible" required>
              <option value="">Выберите пакет сайта</option>
              <option value="Start" data-price="${saleActive ? "5 000 Kč / ≈200 €" : "10 000 Kč / ≈400 €"}" data-regular="${saleActive ? "10 000 Kč / ≈400 €" : ""}">${saleActive ? "START — акция 5 000 Kč / ≈200 € — готовый demo-шаблон" : "START — 10 000 Kč / ≈400 € — готовый demo-шаблон"}</option>
              <option value="Plus" data-price="${saleActive ? "10 000 Kč / ≈400 €" : "20 000 Kč / ≈800 €"}" data-regular="${saleActive ? "20 000 Kč / ≈800 €" : ""}">${saleActive ? "PLUS — акция 10 000 Kč / ≈400 € — больше блоков и доработка" : "PLUS — 20 000 Kč / ≈800 € — больше блоков и доработка"}</option>
              <option value="Pro" data-price="${saleActive ? "от 15 000 Kč / от ≈600 €" : "от 30 000 Kč / от ≈1 200 €"}" data-regular="${saleActive ? "от 30 000 Kč / от ≈1 200 €" : ""}">${saleActive ? "PRO — акция от 15 000 Kč / от ≈600 € — под рекламу и рост" : "PRO — от 30 000 Kč / от ≈1 200 € — под рекламу и рост"}</option>
            </select>
            <input name="package_price_visible" placeholder="Цена пакета" readonly>
            <div class="field-row">
            ...
            <textarea name="message" rows="4" placeholder="${cfg.messagePlaceholder}" required>${saleActive ? `Хочу похожий сайт по акции 50%. Demo: ${cfg.templateName}.` : `Хочу похожий сайт. Demo: ${cfg.templateName}.`}</textarea>
    ...
    <section id="contact">
      <div class="wrap contact-card">
        <div>
          ...
          <h2>Телефон, карта и быстрые кнопки</h2>
          <p class="lead">В реальном проекте сюда ставится адрес, часы работы, телефон, email, мессенджеры и ссылка на карту.</p>
          <div class="contact-list">
            <span>Demo location — Prague / your city</span>
            <span>+420 XXX XXX XXX</span>
            <span>email@yourbusiness.cz</span>
            <a href="#request">Открыть форму заявки</a>
          </div>
        </div>
        <div class="map-box">
          <div>
            <h3 style="color:#fff">DEMO LOCATION</h3>
            <p>Карта подключается под реальный адрес клиента.</p>
            <a class="btn soft" href="#request">Подключить под мой бизнес</a>
          </div>
        </div>
      </div>
    </section>
  ...
  root.innerHTML = `
    ...
    <nav class="bottom-nav" aria-label="Быстрые действия">
      <a href="#offers">Услуги</a>
      <a href="#prices">Цены</a>
      <a href="#request">Заявка</a>
    ...
    <footer>
      <div class="wrap">
        <span>${cfg.brand} — fictional demo template</span>
  ...
  function showSalePopup() {
    ...
    popup.innerHTML = `
      ...
      <strong>Хотите похожий сайт?</strong>
    ...
    popup.querySelector("[aria-label='Закрыть']").addEventListener("click", () => popup.remove());
  ...
  form?.addEventListener("submit", () => {
    ...
    const summaryLines = saleActive
      ? [
        ...
        `Обычная цена: ${form.querySelector("[name='regular_price']").value || "не указана"}`,
