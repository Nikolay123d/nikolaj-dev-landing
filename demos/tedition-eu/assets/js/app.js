...
function scrollToForm(type = "Другой вопрос") {
...
document.querySelectorAll("[data-scroll-form]").forEach((button) => {
  button.addEventListener("click", () => scrollToForm(button.dataset.requestType || "Другой вопрос"));
});

document.querySelector("[data-popup-contact]")?.addEventListener("click", () => scrollToForm("Другой вопрос"));
...
form?.addEventListener("submit", (event) => {
  ...
  if (formStatus) {
    formStatus.textContent = "Спасибо. Это локальное demo - реальную отправку подключим после подтверждения e-mail клиента.";
  }
  form.reset();
});
